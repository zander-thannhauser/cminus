	...
	struct asm_writer* writer)
{
	...
	struct initializer* const initializer = this->initializer;
	ENTER;
	...
	{
		asm_writer_write_lea(writer, -offset, baseptr, rdi);
		asm_writer_write_movi_const_v2(writer, 0, rax, byte);
		asm_writer_write_movi_const_v2(writer, size, rcx, quadword);
		asm_writer_write(writer, "rep stosb");
		return 0;
	}
	...
	asm_writer_write_lea(writer, -offset, baseptr, working_1);
